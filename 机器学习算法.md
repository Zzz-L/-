...
## 模型评价指标
1. 精确率、召回率、F1、ROC曲线、KS曲线
- TP: 分类正确的正样本
  TN: 分类正确的负样本
  FP：分类错误的正样本
  FN：分类错误的负样本
- 精确率： P = TP/(TP+FP) 预测对的正例数占预测正例总量的比值
  召回率：R = TP/(TP+FN) 预测对的正例数占真正正例数的比值
  真阳性率(TPR)：TP/(TP+FN) 在真实为阳性的样本中，被预测为阳性的比例，等同于召回率
  伪阳性率(FPR)：FP/(TN+FP) 在真实为阴性的样本中，被误诊为阳性的比例
- F1：精确率与召回率的调和平均值 2PR/P+R
- ROC曲线：是指当设置不同阈值时，通过绘制TPR与FPR构成的点所得到的曲线，当阈值越接近1，则TPR与FPR接近于0，
  而当阈值越接近于0，则TPR与FPR接近于1，ROC曲线以下的面积表示AUC
- KS曲线：设置不同的阈值，得到一组(TPR,FPR)的差值，当差值最大时，所对应的阈值，则为最优阈值
